...
void draw(cv::Mat img,float confidence_threshold,std::vector<vector<float> > detections,float area,int ox,int oy,int cols,int rows){
      ...
      for (int i = 0; i < detections.size(); ++i) {
        ...
        if (score >= confidence_threshold) {
          //my_code
          leftTop.x=static_cast<int>(d[3] * cols+ox);
          leftTop.y=static_cast<int>(d[4] * rows+oy);
          rightBottom.x=static_cast<int>(d[5] * cols+ox);
          rightBottom.y=static_cast<int>(d[6] * rows+oy);
          if(abs((rightBottom.x-leftTop.x)*(rightBottom.y-leftTop.y))<area){
...
public:
    Controller(){
        num_result=0;
        num_detector=0;}
    bool add_detector(Detector& d){
        if(num_detector>=NUM_DET){return false;}
        dt[num_detector]=&d;
        ++num_detector;
        return true;
    }
    //void mul_detect(std::vector<vector<float> >* result,Detector* dt,)
    void crab_result(cv::Mat &img,int row_sum,int col_sum){
        //thread td[NUM_DET];
        int detector_step=row_sum*col_sum/NUM_DET;
        int det_num=0;
        cv::Mat mask;
        ...
        result[num_result]=dt[0]->Detect(img);
        ++num_result;
    }
    void draw_result(cv::Mat &img,int row_sum,int col_sum,float confidence_threshold){
        cv::Mat mask;
        int width = img.size().width;
        int height = img.size().height;
        for(int i=0;i<row_sum;i++){
          for(int k=0;k<col_sum;k++){
            //cv::Rect rec(width/col_sum*k,height/row_sum*i,width/col_sum,height/row_sum);
            //mask = img(rec);
            draw(img,confidence_threshold,result[get_num_result()-col_sum*row_sum-1+i*col_sum+k],area_in,width/col_sum*k,height/row_sum*i,width/col_sum,height/row_sum);
          }
        }
        draw(img,confidence_threshold,result[get_num_result()-1],area_out,0,0,width,height);
    }
    int get_num_result(){return num_result;}
    std::vector<vector<float> >* get_result(){return result;}
    int get_num_detector(){return num_detector;}
    void view(cv::Mat img){
...
private:
    int num_detector;
    Detector* dt[20];
    std::vector<vector<float> > result[1000];
    int num_result;
};
...
gettimeofday(&t1,NULL);
  ...
  cout<<"num_detector: "<<ctl.get_num_detector()<<endl;
  ...
  while (infile >> file) {
    if (file_type == "image") {
      ...
      //my_code
      ctl.crab_result(img,row_sum,col_sum);
      //std::vector<vector<float> >* result=ctl.get_result();
      cout<<"num_result: "<<ctl.get_num_result()<<endl;
      gettimeofday(&t4,NULL);
      t_all = (t4.tv_sec-t3.tv_sec)*1000000 +t4.tv_usec-t3.tv_usec;
      t_all /= 1000000;
      cout<<t_all<<endl;
      ctl.draw_result(img,row_sum,col_sum,confidence_threshold);
      cout<<ctl.get_num_result()<<endl;
      ctl.view(img);
      //exit(0);
      //end
      ...
      int frame_begin=1000,frame_end=1500;
      std::vector<vector<float> > detections;
      while(frame_count<frame_begin)
      {
        bool success = cap.read(img);
        if (!success) {LOG(INFO) << "Process " << frame_count << " frames from " << file;break;}
        CHECK(!img.empty()) << "Error when read frame";
        if (frame_count<frame_begin){frame_count++;continue;}
      }
      while (frame_count<frame_end) {
        ...
        CHECK(!img.empty()) << "Error when read frame";
        gettimeofday(&t3,NULL);
        if(frame_count%1==0){ctl.crab_result(img,row_sum,col_sum);}
        //cout<<"num_result: "<<ctl.get_num_result()<<endl;
        gettimeofday(&t4,NULL);
        t_all = (t4.tv_sec-t3.tv_sec)*1000000 +t4.tv_usec-t3.tv_usec;
        t_all /= 1000000;
        cout<<"single_time: "<<t_all<<"  FPS: "<<1/t_all<<endl;
        ctl.draw_result(img,row_sum,col_sum,confidence_threshold);
        //ctl.view(img);
