...
gettimeofday(&t1,NULL);
  ...
  Detector detector4(model_file, weights_file, mean_file, mean_value);
  ...
  while (infile >> file) {
    if (file_type == "image") {
      ...
      int flagsss;
      pid_t pid=9;
      cout<<"this is father process,id="<<getpid()<<",start to process "<<endl;
      for(int i=0;i<row_sum;i++){
          ...
          if(pid==0||pid==-1)
        {
          ...
          if(pid==0){
          if(flagsss==0){
            cout<<"this is children process,id="<<getpid()<<",start to process ";
            mul_thread_show(mask,confidence_threshold,detector,false);
            exit(0);
          }
          if(flagsss==1){
            cout<<"this is children process,id="<<getpid()<<",start to process ";
            mul_thread_show(mask,confidence_threshold,detector2,false);
            exit(0);
          }
          if(flagsss==2){
            cout<<"this is children process,id="<<getpid()<<",start to process ";
            mul_thread_show(mask,confidence_threshold,detector3,false);
            exit(0);
          }
          if(flagsss==3){
            cout<<"this is children process,id="<<getpid()<<",start to process ";
            mul_thread_show(mask,confidence_threshold,detector4,false);
          }
          }
      ...
      //getchar();
      //end
      //mul_thread_show(img,confidence_threshold,detector4,true);
      exit(0);
